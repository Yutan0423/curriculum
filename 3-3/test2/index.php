...
// require_once()はファイルを読み込めなくてエラーが発生したらそこで止める
// include_once()はファイルが読み込めなくてエラーが発生しても止めない
...
if (isset($_POST["signUp"])) {
    ...
    if (!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["password2"]) && $_POST["password"] === $_POST["password2"]) {
        ...
        try {
            ...
            // prepare関数の引数にSQL文をぶちこむ
            $stmt = $pdo->prepare("INSERT INTO users(name, password) VALUES (:name, :password)");
            $stmt->execute(array(':name' => $username, ':password' => password_hash($password, PASSWORD_DEFAULT)));  // パスワードのハッシュ化を行う
